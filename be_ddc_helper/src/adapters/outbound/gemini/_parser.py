...
def parse_section_plan(response_text: str) -> SectionPlan:
    try:
        data = json.loads(response_text)
    except json.JSONDecodeError as error:
        raise LLMOutputParseError(f"Gemini response is not valid JSON: {error}") from error

    raw_sections = data.get("sections")
    if not isinstance(raw_sections, list):
        raise LLMOutputParseError("Expected 'sections' list in Gemini response")
